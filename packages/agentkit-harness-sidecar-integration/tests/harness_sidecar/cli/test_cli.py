...
from agentkit.extensions.harness_sidecar.cli import _config, harness_app
...
def test_core_deploy_command_does_not_expose_sidecar_vpc_flags() -> None:
    ...
    parameters = inspect.signature(deploy_command).parameters
    assert "runtime_vpc_id" not in parameters
    assert "runtime_subnet_id" not in parameters
