...
| `--mcp-toolset-id` | 绑定 AgentKit MCP Toolset，下次部署 harness 时注入 `MCP_TOOLSET_ID` 并作为 Runtime 关联资源生效。 | `--mcp-toolset-id mcp-ts-xxx` |
...
- 示例：`agentkit invoke harness my-harness --mcp-toolset-id mcp-ts-xxx "帮我查询订单"`
