...
| `--mcp-toolset-id` | 绑定 AgentKit MCP Toolset，下次部署 harness 时注入 `MCP_TOOLSET_ID` 并作为 Runtime 关联资源生效。 | `--mcp-toolset-id mt-xxx` |
...
`--mcp-toolset-id` 单次 MCP Toolset 覆写参数
- 作为本次调用的 `harness.mcp_toolset_id` 发送给 Harness Runtime，不写回 spec，也不修改已部署 Runtime 的默认绑定
- 示例：`agentkit invoke harness my-harness --mcp-toolset-id mt-xxx "帮我查询订单"`
