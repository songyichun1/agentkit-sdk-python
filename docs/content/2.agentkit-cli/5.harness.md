...
| `--mcp-toolset-id` | 绑定 AgentKit MCP Toolset，部署后作为 Runtime 关联资源生效。 | `mt-xxx` |
...
如果 Harness 需要使用 AgentKit MCP Toolset，可以在本地 spec 中绑定 Toolset ID：

```bash
agentkit add harness --name my-harness \
  --mcp-toolset-id mt-xxx
```

部署时该字段会同时作为 Runtime 关联资源下发，并注入 `MCP_TOOLSET_ID` 环境变量。Harness Runtime 会用这个 ID 查询 AgentKit MCP Toolset 的连接信息，并把对应的 MCP 服务挂载为可执行工具；修改后需要重新部署才会影响云端运行时。
...
# 绑定 MCP Toolset（下次 deploy --harness 后生效）
agentkit add harness --name my-harness \
  --mcp-toolset-id mt-xxx
...
| `--mcp-toolset-id` | 覆写本次调用使用的 AgentKit MCP Toolset。该值只随本次请求发送，不写回 spec。 | — |
