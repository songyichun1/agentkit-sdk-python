...
from agentkit_identity.errors import WorkloadBindingError
...
class ProtectedTarget:
    ...
    def __post_init__(self) -> None:
        ...
        if parsed.username or parsed.password or parsed.query or parsed.fragment:
            raise ValueError(
                "protected target base_url cannot contain credentials, "
                "query, or fragment"
            )
...
class IdentityRuntimeConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if bool(self.workload_discovery_url) != bool(self.expected_workload_issuer):
            raise WorkloadBindingError(
                "workload token discovery and expected issuer must be "
                "configured together"
            )
